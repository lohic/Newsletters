...
class Organisme {
	...
	*/
	function create_organisme($_array_val,$_id=NULL){
		...
		if(isset($_id)){
			//MODIFICATION
			$updateSQL 		= sprintf("UPDATE organisme_tb SET nom=%s, google_analytics_id=%s WHERE id=%s",
			...
			//CREATION
			$insertSQL 		= sprintf("INSERT INTO organisme_tb (nom, google_analytics_id) VALUES (%s,%s)",
													GetSQLValueString($_array_val['nom'],					"text"),
													GetSQLValueString($_array_val['google_analytics_id'], 	"text"));
	...
	*/
	function create_user_groupe($_array_val,$_id=NULL){
		...
		if(isset($_id)){
			//MODIFICATION
			$updateSQL 		= sprintf("UPDATE user_groupes_tb SET libelle=%s, id_organisme=%s WHERE id=%s",
													GetSQLValueString($_array_val['nom'],			"text"),
			...
			//CREATION
			$insertSQL 		= sprintf("INSERT INTO user_groupes_tb (libelle, id_organisme) VALUES (%s,%s)",
													GetSQLValueString($_array_val['nom'],			"text"),
													GetSQLValueString($_array_val['id_organisme'], 	"text"));
			$insert_query	= mysql_query($insertSQL) or die(mysql_error());
	...
	*/
	function get_organisme_edit_liste(){
		...
		$sql_organisme		= sprintf('SELECT * FROM organisme_tb');
	...
	*/
	function get_user_groupe_edit_liste(){
		...
		$sql_user_groupe		= sprintf('SELECT * FROM user_groupes_tb');
	...
	*/
	function suppr_organisme($id=NULL){
		if(isset($id)){
			...
			$supprSQL		= sprintf("DELETE FROM organisme_tb WHERE id=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
			
			
			$sql_user_groupe		= sprintf("SELECT * FROM user_groupes_tb WHERE id_organisme=%s", GetSQLValueString($id,'int'));
			$sql_user_groupe_query = mysql_query($sql_user_groupe) or die(mysql_error());
	...
	*/
	function suppr_user_groupe($id=NULL){
		if(isset($id)){
			...
			$supprSQL		= sprintf("DELETE FROM user_groupes_tb WHERE id=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
			
			$supprSQL		= sprintf("DELETE FROM rel_user_groupe_tb WHERE id_groupe=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
			
			$supprSQL		= sprintf("DELETE FROM rel_user_groupe_groupe_tb WHERE id_groupe=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
			
			$supprSQL		= sprintf("DELETE FROM rel_template_groupe_tb WHERE id_groupe=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
			
			$supprSQL		= sprintf("DELETE FROM rel_cat_actu_groupe_tb WHERE id_groupe=%s", GetSQLValueString($id,'int'));
			$suppr_query	= mysql_query($supprSQL) or die(mysql_error());
		}
	}
	...
	*/
	function get_admin_level(){
		$sql_liste_level	= 'SELECT * FROM user_level_tb ORDER BY level';

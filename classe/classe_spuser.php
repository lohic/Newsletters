...
class Spuser {
	...
	function isLDAP($login=NULL,$password=NULL){
		...
		if(IS_LDAP_SERVER){
			...
			if(!empty($LDAPinfo->email)){
				...
				$login_query	= sprintf("SELECT * FROM user_tb WHERE email=%s", GetSQLValueString($LDAPinfo->email, "text")); 
	...
	*/
	function modify_user($_array_val=NULL){
		if($this->isAdmin){
			...
			$updateSQL 		= sprintf("UPDATE user_tb	SET login=%s,
															password=%s,
															type=%s,
															nom=%s,
															prenom=%s,
	...
	*/
	function add_user($_array_val=NULL){
		if($this->isAdmin){
			...
			$this->connexion->connect_db();
			$updateSQL 		= sprintf("INSERT INTO user_tb	(login,password,type,nom,prenom,email,account_type) VALUES (%s,%s,%s,%s,%s,%s,%s)",
													GetSQLValueString($_array_val['login'], "text"),
									   				GetSQLValueString($_array_val['password'], "text"),
									   				GetSQLValueString($_array_val['type'], "text"),
									   				GetSQLValueString($_array_val['nom'], "text"),
	...
	function add_groupe_user($_id_user=NULL, $_array_groupe=NULL){
		if(!empty($_id_user) && !empty($_array_groupe)){
			...
			foreach($_array_groupe as $_id_groupe){
				$insertSQL 		= sprintf("INSERT INTO rel_user_groupe_tb (id_user,id_groupe) VALUES (%s,%s)",
	...
	*/
	function clean_groupe($_id_user=NULL){	
		if(!empty($_id_user)){
			...
			$supprSQL		= sprintf("DELETE FROM rel_user_groupe_tb WHERE id_user=%s", GetSQLValueString($_id_user,'int'));
	...
	*/
	function suppr_user($_id_user=NULL){	
		if(!empty($_id_user)){
			...
			$this->clean_groupe($_id_user);
			$supprSQL		= sprintf("DELETE FROM user_tb WHERE id=%s", GetSQLValueString($_id_user,'int'));
	...
	*/
	function get_groups(){		
		$query	= sprintf("SELECT g.id as id, g.libelle AS libelle, g.type AS type, g.id_organisme AS id_organisme
							FROM user_groupes_tb g, user_tb u, rel_user_groupe_tb r
	...
	*/
	function get_user_list(){
		$sql_liste_user = 'SELECT * FROM user_tb ORDER BY account_type DESC, nom';
	...
	*/
	function get_groupe($_id=NULL,$id_user=NULL){
		...
		while ($groupe = mysql_fetch_assoc($sql_liste_groupe_query)){
			...
			if(isset($id_user)){
				$sql_liste_user	= "SELECT * FROM rel_user_groupe_tb WHERE id_groupe=".$groupe['id'];
				$sql_liste_user_query = mysql_query($sql_liste_user) or die(mysql_error());

...
class Newsletter {
	...
	function get_template($id, $short=false ){
		if($id){
			...
			if($short){
				...
				$adresse = 'http://localhost:8888/Site_SCIENCESPO_NEWSLETTER/admin-new/template/'.$template_folder['template'].'/';
			}else{
				$adresse = 'http://'.$_SERVER["SERVER_NAME"].'/newsletter/admin-new/template/'.$template_folder['template'].'/';
			}
			return $adresse;
		}
	}
	...
	function add_newsletter_item($_origine,$_id_origine,$_ordre,$_liste_name){
		...
		$sql_liste_events		= sprintf('SELECT evenement_id AS id,
			...
			$insertSQL 		= sprintf("INSERT INTO newsletter_item_tb (origine,id_origine,ordre,id_newsletter,liste_name,date1,horaire,lieu,titre,soustitre,texte,info,image,moreTXT,URL) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
	...
	function get_newsletters_liste($id_template){

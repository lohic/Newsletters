...
if(isset($_POST['update']) && $_POST['update']=='actu'){
	...
	if(!empty($_POST['id'])){
		...
		if(isset($_FILES['picturefile']) && !empty($_FILES['picturefile']['name']) && $_FILES['picturefile']['name']!=''){
			...
			if($_FILES['picturefile']['type'] == "image/jpeg" || $_FILES['picturefile']['type'] == "image/jpg"){
				resize($fileName,'../actu_images/'.$id_actu.'/', 512, 1024);
			}
			if($fileName){
				$tab_actu['image']		= $fileName;
			}else{
    ...
    <form action="" method="post" enctype="multipart/form-data" id="actu_form">
    	<fieldset>
            ...
            <p>
                ...
                <input type="text"  class="inputField" name="titre" id="titre" value="<?php echo $info_actu['titre'];?>" />
            </p>
            <p>
            	<label for="categorie" class="inline">Catégorie :&nbsp;</label>
				<?php echo createSelect($actu->get_liste_cat(), 'categorie'	, $info_actu['categorie'], '', false);?>
            </p>
        </fieldset>
        <fieldset>
            <p>
                <label for="soustitre_actu" class="inline">Sous titre (max 255 caractères) :</label>
                <textarea type="text" name="soustitre" id="soustitre_actu" class="textareaField" ><?php echo $info_actu['soustitre'];?></textarea>
            </p>
            <p>
        ...
        <fieldset>
            <p>
                ...
                <input class="inputField"type="text" name="moreTXT" id="moreTXT" value="<?php echo $info_actu['moreTXT'];?>" />
            </p>
            <p>

<!-- //////////////// -->
<!-- BLOC DE NEWS     -->
<!-- //////////////// -->
<tr>
    <td width="15" height="10"><a name="item-<?php echo $id;?>" id="item-<?php echo $id;?>"></a></td>
    <td width="15" height="10" bgcolor="#FFFFFF"></td>
    <td height="10" colspan="2" bgcolor="#FFFFFF"><?php if(isset($image)){?><img src="<?php echo $image?>" alt="photo" width="740" /><?php } ?></td>
    <td width="15" height="10" bgcolor="#FFFFFF"></td>
    <td width="15" height="10"></td>
  </tr>
  <tr>
    <td width="15" height="20">&nbsp;</td>
    <td width="15" height="20" bgcolor="#FFFFFF">&nbsp;</td>
    <td height="20" colspan="2" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="15" height="20" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="15" height="20">&nbsp;</td>
  </tr>
  
  
  
  <tr>
    <td width="15" height="25" background="<?php echo $template;?>images/titre-rectangle.gif">&nbsp;</td>
    <td width="15" bgcolor="#da2e00">&nbsp;</td>
    <td width="26" bgcolor="#da2e00">&nbsp;</td>
    <td width="714" bgcolor="#FFFFFF"><h2 style="font-family:Arial, Helvetica, sans-serif; text-transform:uppercase; font-size:24px; margin:0 0 0 15px; font-weight:normal; color:#da2e00"><?php echo $titre?></h2></td>
    <td width="15" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="15">&nbsp;</td>
  </tr>
  <tr>
    <td width="15" height="7" background="<?php echo $template;?>images/titre-triangle.gif"></td>
    <td width="15" height="7" bgcolor="#FFFFFF"></td>
    <td height="7" colspan="2" bgcolor="#FFFFFF"></td>
    <td width="15" height="7" bgcolor="#FFFFFF"></td>
    <td width="15" height="7"></td>
  </tr>
  <tr>
    <td width="15" height="10"></td>
    <td width="15" height="10" bgcolor="#FFFFFF"></td>
    <td height="10" colspan="2" bgcolor="#FFFFFF"></td>
    <td width="15" height="10" bgcolor="#FFFFFF"></td>
    <td width="15" height="10"></td>
  </tr>
  <tr>
    <td width="15">&nbsp;</td>
    <td width="15" bgcolor="#FFFFFF">&nbsp;</td>
    <td colspan="2" valign="top" bgcolor="#FFFFFF"><div style="font-family:Arial, Helvetica, sans-serif; font-size:14px; color:#676767"><?php echo $texte?></div>
      <?php if($origine == "evenement_db"){ ?>
                <p style="font-family:Arial, Helvetica, sans-serif; font-size:10px"><span style="color:#da2e00">&gt;&gt;</span>&nbsp;<a href="http://capricorne.sciences-po.fr/evenements/?eventId=<?php echo $id_event;?>&fromnews=<?php echo $news_id; ?>" target="_blank" style="color:#676767">Read more</a></p>      
            <?php }else if($origine == "evenement_new_db"){ ?>
                <p style="font-family:Arial, Helvetica, sans-serif; font-size:10px"><span style="color:#da2e00">&gt;&gt;</span>&nbsp;<a href="<?php echo $URL;?>&fromnews=<?php echo $news_id; ?>" target="_blank" style="color:#676767">Read more</a></p>
            <?php }else if(isset($URL)){ ?>
                <p style="font-family:Arial, Helvetica, sans-serif; font-size:10px"><span style="color:#da2e00">&gt;&gt;</span>&nbsp;<a href="<?php echo $URL;?>" target="_blank" style="color:#676767">Read more</a></p>
             <?php } ?></td>
    <td width="15" bgcolor="#FFFFFF">&nbsp;</td>
    <td width="15">&nbsp;</td>
  </tr>
<!-- //////////////// -->
<!-- FIN DE BLOC DE NEWS  -->
<!-- //////////////// -->